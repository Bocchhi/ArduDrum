...
typedef struct {
  ...
  byte BP_ScanTimeBounce; //Masking done with this
  byte BP_AnalogPin;
...
bool compare_to_threshold(int *valueArr, int threshold);
...
const unsigned int BasePiezoDynamicValueCounter = 2; //bounced, velocity; dynamic value sayısı sayacı
const unsigned int SinglePiezoTCRTDynamicValueCounter = 1; //Hi-Hat pedal last CC

byte Default_Threshold = 150; //Max is 255 since it can't exceed byte size
byte Default_ScanTimePeak = 4; //Actual was 8
byte Default_ScanTimeBounce = 17; //Actual was 24
...
unsigned int BasePiezoCounter = 0; //This will increase 1 every time create_BP trgiggered so it will be equal to number of BasePiezo element
BasePiezo *BasePiezoArray = NULL; //This will store the all the created BasePiezo elements with create function called
int **BasePiezoDynamicValueTracker = NULL; //[0][0]=Snare velocity [0][1]=Snare bounced
...
void setup() {
  ...
  for(int i=0; i<BasePiezoCounter; i++){
    BasePiezoDynamicValueTracker[i][BPDVI_BOUNCED]=BOUNCED_TRUE; //İnitializing bounced value to ture for every Index
    BasePiezoDynamicValueTracker[i][BPDVI_VELOCITY]=MIDI_NOTE_OFF_VELOCITY; //İnitializing velocity value to off for every Index
  }
...
BasePiezo create_BP(byte *BP_Attributes) {
  BasePiezo bp = {
    ...
    .BP_ScanTimePeak = *(BP_Attributes + 1),
    .BP_ScanTimeBounce = *(BP_Attributes + 2),
  ...
  BasePiezoDynamicValueTracker = (int**)realloc(BasePiezoDynamicValueTracker, BasePiezoCounter*sizeof(int*));
  BasePiezoDynamicValueTracker[BasePiezoCounter-1] = (int*)malloc(BasePiezoDynamicValueCounter*sizeof(int));
  return bp;
}
...
//Comparing if all the given values are smaller than the treshold
bool compare_to_threshold(int *valueArr, int threshold){
  bool flag=true;
  int significantElementNumber = calculate_significant_element_number(valueArr);
  for(int i=0; i<significantElementNumber; i++){
    if(*(valueArr+i)<threshold)
      continue;
    else{
      flag=false;
      break;
...
//For bounce we check if the all the values are below threshold throughout the entire scanning period.
void check_bounced_BP(BasePiezo bp){
  ...
  //if bounces is not already true 
  int CurrentPiezoValue_BP = analogRead(bp.BP_AnalogPin);
  //if current value is below threshold we start scan, else we will get false anyway so we don't lost time with scanning an already false situation
  if(CurrentPiezoValue_BP<bp.BP_Threshold){
    //Scan started
    int *values_BP = scan_BP(bp, bp.BP_ScanTimeBounce);
    bool CompareResult_BP = compare_to_threshold(values_BP, bp.BP_Threshold);
    free(values_BP);

    if(CompareResult_BP){
      //Serial.print("Bounce changed to true\n");
      BasePiezoDynamicValueTracker[IndexNo_BP][BPDVI_BOUNCED]=BOUNCED_TRUE;
      return;
    }
    //Compare result failed so we didn't change the bounce state which means it stays as false
    else
      return;
  }

  //Current value is bigger than threshold so we skipped if block and the scan didn't start and we finished function as the bounce state didn't change and stayed same which is false
  return;
}
...
int sense_DP(DoublePiezo dp){
  ...
  if(CurrentPiezoValue_BP1>bp1.BP_Threshold || CurrentPiezoValue_BP2>bp2.BP_Threshold){
    ...
    //If dp doesn't work set other sp bounced to false as well
